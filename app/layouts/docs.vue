...
interface NavItem { title: string; path: string }
interface NavSection { title: string; folderLabel?: string; items: NavItem[] }
      ...
      <aside
        ...
        <nav class="space-y-7">
          <div v-for="section in navSections" :key="section.title">
            ...
            <!-- Section header -->
            <div class="mb-2 px-3">
              <p class="text-foreground text-xs font-semibold tracking-widest uppercase">{{ section.title }}</p>
              <code v-if="section.folderLabel"
                class="text-muted-foreground/60 font-mono text-[10px]">{{ section.folderLabel }}</code>
            </div>
      ...
      <div class="flex min-w-0 flex-1">
        ...
        <main class="min-w-0 flex-1 px-8 py-10">
          <div class="mx-auto max-w-3xl">
            ...
            <nav v-if="prevItem || nextItem" aria-label="Page navigation"
              class="border-border mt-16 flex items-stretch justify-between gap-4 border-t pt-8">
              <NuxtLink v-if="prevItem" :to="prevItem.path"
        ...
        <aside v-if="toc?.links?.length"
          class="sticky top-14 hidden h-[calc(100vh-3.5rem)] w-52 shrink-0 overflow-y-auto py-8 pl-4 pr-4 xl:block">
          <p class="text-muted-foreground mb-3 text-xs font-semibold tracking-widest uppercase">On this page</p>
          <ul class="space-y-0.5">
            <template v-for="link in toc.links" :key="link.id">
              <li>
                <a :href="`#${link.id}`"
                  class="text-muted-foreground hover:text-foreground block py-1 text-sm transition-colors">{{ link.text
                  }}</a>
              </li>
